CREATE OR REPLACE FUNCTION public.member_update_own_record(
  ...
  _young_eagle_pilot boolean DEFAULT NULL,
  _young_eagle_volunteer boolean DEFAULT NULL,
  _eagle_pilot boolean DEFAULT NULL,
  _eagle_flight_volunteer boolean DEFAULT NULL
)
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  UPDATE roster_members SET
    ...
    young_eagle_pilot = COALESCE(_young_eagle_pilot, young_eagle_pilot),
    young_eagle_volunteer = COALESCE(_young_eagle_volunteer, young_eagle_volunteer),
    eagle_pilot = COALESCE(_eagle_pilot, eagle_pilot),
    eagle_flight_volunteer = COALESCE(_eagle_flight_volunteer, eagle_flight_volunteer),
    updated_at = now()
